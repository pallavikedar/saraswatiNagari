...
// Project name constant - updated for project 10
const PROJECT_NAME = "Saraswati Nagari-11"; // Changed for project 10
...
const Map11 = () => {
  ...
  // Fetch data from API - CHANGED to project 10
  useEffect(() => {
    let cancelled = false;
    const fetchPlots = async () => {
      try {
        setLoading(true);
        setError(null);
        // UPDATED API ENDPOINT from /1 to /10
        const response = await fetch("https://api.sgroup.space/layouts/project/10");
        ...
        console.error("Error fetching project 10 data:", err);
      } finally {
            ...
            {data.map((plot) => {
              ...
              return (
                ...
                >
                  ...
                  <div>
                    <span className={`pm-badge ${plot.status}`}>
                      {plot.rawStatus}
                    </span>
                  </div> */}
                </div>
